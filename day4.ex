...
defmodule Calc do
  ...
  def isSubRange?(rangeA, rangeB) do
    if (Calc.inRange?(Enum.at(rangeA, 0), rangeB) and inRange?(Enum.at(rangeA, 1), rangeB))
      or (Calc.inRange?(Enum.at(rangeB, 0), rangeA) and inRange?(Enum.at(rangeB, 1), rangeA)) do
      # IO.puts("in true")
      true
    else
      # IO.puts("in else")
...
List.foldl(pairs, 0, fn pair, accu ->
  ...
  if length(pairRanges) == 2 do
    ...
    if Calc.isSubRange?(taskARangeParsed, taskBRangeParsed) do
      IO.puts("in if")
      IO.puts(accu)
      accu + 1
    else
